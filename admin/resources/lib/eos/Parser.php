...
class Parser {
    ...
    protected $ST2 = array('+', '-');
    //Allowed functions
    protected $FNC = array('sin', 'cos', 'tan', 'csc', 'sec', 'cot', 'abs', 'log', 'log10', 'sqrt', 'ceil');
    /**#@-*/
    /**
     * Construct method
     *
     ...
     */
    function solveIF($infix, $vArray = null, $inWhile = false) {
        if ($inWhile) {
            //echo 'tak';
        } else {
            //echo 'false';
...
//	    var_dump($vArray);
        ...
        while((preg_match("/(". implode("|", $this->FNC) . ")\(([^\)\(]*(\([^\)]*\)[^\(\)]*)*[^\)\(]*)\)/", $infix, $match)) != 0) {
            $func = $this->solveIF($match[2], null, true);
            switch($match[1]) {
                case "cos":
                    $ans = cos($func);
                    break;
...
//                    break;
        ...
        $price = $this->solvePF($this->in2post($infix));
        if (!$inWhile) {
            $rabaty = [
                1 => [
                    'method' => function($metraz, $cena) {
                        if ($metraz > 100) {
                            return $cena * 0.20;
                        } else if ($metraz > 50) {
                            return $cena * 0.15;
            ...
            if (preg_match("/rabat\((.*)\)/", $infix) !== false) {
                //liczenie rabatu

                $metraz = 0;
                if (isset($vArray['aws_price_calc_1']) && $vArray['aws_price_calc_2']) {
                    $metraz = ($vArray['aws_price_calc_1'] * $vArray['aws_price_calc_2']) / 10000;
                }

                if (isset($vArray['aws_price_calc_34']) && $vArray['aws_price_calc_35']) {
                    $metraz = ($vArray['aws_price_calc_34'] * $vArray['aws_price_calc_35']) / 10000;
                }


                if (preg_match("ceil", $infix) !== false) {
                    $metraz = ceil($metraz);
                }
                preg_match_all("/rabat\((.*)\)/",
                    $infix,
                    $salida);
                $numerRabatu = $salida[1][0];

                // maly hack
                $price = $price - $numerRabatu;
                if ($numerRabatu > 0 && isset($rabaty[$numerRabatu])) {
                    $price = round($price - $rabaty[$numerRabatu]['method']($metraz, $price), 2);
                }
            }
            ...
            /// minimalna cena dla etykiet
            if ($product->id == 3573 && $price < 50) {
                $price = 50;
            }
        }
        //var_dump($price);
